...
- **崩溃录音恢复**：录音期间持续写入 PCM journal；异常退出后可在历史记录中恢复音频、播放，并用当前识别引擎重新转写
- **顶部双栏转写面板**：在目标窗口所在屏幕顶部并排展示原始转写与优化结果，支持折叠、录音计时和 LLM 请求耗时
- **Type4Me Lab 品牌**：定制构建使用独立的深色声波「4」图标和显示名称；Bundle ID、历史数据目录与权限身份保持不变
...
- **浮窗崩溃修复**：面板尺寸统一由 AppKit 控制并延迟到显示周期外更新，避免 SwiftUI `NSHostingView` 在窗口布局中递归触发约束异常
- **录音终态与优化失败恢复**：无文本录音使用明确终态收起面板；最终优化失败可重试或插入原文，并保留 ESC 与超时兜底
- **历史音频保留上限**：按 30 天、500 条和 1 GB 三重上限清理音频，并在启动及删除后核对数据库引用与孤立文件
- **紧凑毛玻璃面板**：顶部双栏缩小正文、标题和状态字号，收紧栏头、边距、行距与按钮尺寸；烟熏遮罩进一步降低到 22%–40%，让真实窗口背景更明显透出
- **录音到处理无闪烁**：停止录音后沿用同一个可见 panel，仅原位更新状态与内容，不再清空透明度、重建视图后重新淡入
- **短语实时优化**：短句达到 4 个有效字符并稳定停顿后即可触发右栏优化；关闭当前机器原有的“10 字以下跳过”设置，并修正等待阶段误显示“已同步”的状态
- **面板模式切换**：顶部模式名称改为可点击下拉菜单，仅展示当前识别引擎支持的录音模式，并附带已配置快捷键；录音中切换会清除旧模式预览并用新 Prompt 继续处理，处理阶段锁定选择避免结果错配
- **实时优化限流保护**：首轮等待 4 个有效字符并停顿 1.2 秒，后续请求至少间隔 5 秒且累计新增 20 字；每次录音最多 3 次预览，忽略标点/空格与轻微 ASR 抖动，HTTP 429 后立即熔断实时请求并保留最终优化
- **最终润色权威调用**：录音中的 LLM 结果仅作预览；停止录音后始终使用最终 ASR 文本重新调用一次 LLM，不再直接复用可能过时或质量不佳的预览结果
- **过期预览清理**：ASR 发生整体改写时立即清空不匹配的右栏结果；仅在原文追加尾部内容时保留旧结果并标记“待更新”
- **润色输出边界**：用 `<speech_transcript>` 隔离口述内容，明确禁止“好的 / 收到 / 我会”等对话式回应、规则说明和分隔线；命令句只做文字整理，不执行也不回应
- **完整 ASR 终态**：停止录音后先等待服务端事件流结束；分句级 `isFinal` 不再被误当成整段完成。结束或 drain 失败时先用保存的完整录音重识别，仍失败则保留音频并拒绝用残缺文本调用 LLM
- **短文本也执行最终润色**：移除“短文本跳过”设置；只要当前模式需要 LLM 且最终文本非空，停止后必定发起权威请求
- **元回复输出门禁**：语音润色结果若新增“好的，我会…”等对话式确认语会被拒绝；最终请求自动严格重试 1 次，仍违规则进入失败恢复，不会直接插入
- **紧凑浮窗**：双栏面板改用接近参考图的深灰蓝底色；单行高度约 85–105 pt，缩减外边距、标题栏、正文留白、按钮和失败操作区占用
- **实时预览状态一致性**：流式 ASR 在分句确认时新增内部标点不再清空已完成优化稿；无语音时右栏改为“等待语音”，空结果时顶部状态不再错误显示上一轮 LLM 耗时
- **录音界面交互**：屏幕底部 Siri 光球移除中央音量条，仅以惯性平滑后的缩放、色彩流动和外环反馈说话强度；底部 HUD 除撤销按钮外可从任意位置直接拖动，顶部双栏面板缩小后也可拖动；两者在内容更新时不会吸回默认位置，隐藏后再次显示会恢复各自上次位置
- **耳机中键稳定性**：改为直接读取 AppleMikey 原始 HID 报告，移除会因丢失 key-up 而永久连发的 F20 映射；将一次按下/松开或长按归一化为一次录音切换，音量加减键继续透传给系统
- **耳机线控配置与反馈**：模式设置可将上键、中键、下键分别分配给任意模式，或一键设为三键合一；每次识别到完整按压都会显示短暂的非打扰提示
- **悬浮快捷键面板**：新增始终置顶、可拖动且位置持久化的虚拟键盘面板；最多配置 6 个按钮，支持键盘组合键、按住/松开语义，并可随时关闭或从通用设置重新开启
- **上游稳定性修复同步**：按 CoreAudio 返回大小分配输出设备缓冲区，避免多缓冲 USB 设备越界；复用并正确关闭 HistoryStore 数据库连接；内部剪贴板恢复标记为临时写入且不覆盖并发复制；提示词上下文改为按需异步捕获，不再阻塞录音启动
