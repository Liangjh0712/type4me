...
final class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    // Import crash-interrupted audio locally before history is shown. This never
    // calls an ASR service; users explicitly retry from the history screen.
    Task {
      let recovered = await Task.detached {
        AudioArchive.shared.recoverInterruptedRecordings()
      }.value
      for item in recovered {
        let partial = item.metadata.partialTranscript
        if await session.historyStore.contains(id: item.metadata.recordID) {
          AudioArchive.shared.commitRecoveredMetadata(recordID: item.metadata.recordID)
          continue
        }
        let inserted = await session.historyStore.insert(
          HistoryRecord(
            id: item.metadata.recordID,
            createdAt: item.metadata.createdAt,
            durationSeconds: item.audio.durationSeconds,
            rawText: partial,
            processingMode: item.metadata.processingMode,
            processedText: nil,
            finalText: "",
            status: "crash_recoverable",
            characterCount: partial.count,
            asrProvider: item.metadata.asrProvider,
            asrModel: item.metadata.asrModel,
            audioPath: item.audio.relativePath,
            audioBytes: item.audio.byteCount,
      ...
      await session.historyStore.pruneAudio()
      await session.historyStore.reconcileAudioArchive()
    }
    let appState = self.appState
    appState.onStopRequested = { [weak self] in
    ...
    appState.onCancelRequested = { [weak self] in
      guard let self else { return }
      let phase = self.appState.barPhase
      guard phase == .preparing || phase == .recording else { return }
    ...
    appState.onPanelModeSelected = { [weak self] mode in
      guard let self else { return }
      let previous = self.panelModeSessionTask
      let next = Task {
        await previous?.value
        await session.switchMode(to: mode)
      }
      self.panelModeSessionTask = next
    }
    appState.onRetryFinalOptimization = {
      Task { await session.retryFinalOptimization() }
    }
    appState.onInsertRawAfterFailure = {
      Task { await session.insertRawAfterOptimizationFailure() }
    }
    ...
    Task { @MainActor in
      for await event in asrEventStream {
        ...
        case .ready:
          ...
          DebugFileLogger.log(
            "ready event received, current barPhase=\(String(describing: appState.barPhase))")
          appState.markRecordingReady()
          Task { @MainActor in
            guard appState.barPhase == .recording else {
              DebugFileLogger.log(
                "playStart aborted, barPhase=\(String(describing: appState.barPhase))")
              return
            }
        ...
        case .llmRequestStarted(let provider, let model, let attempt):
          appState.showLLMCallStarted(provider: provider, model: model, attempt: attempt)
        case .llmRequestFinished(let provider, let model, let attempt, let duration, let succeeded):
        ...
        case .finalOptimizationFailed(let message, let sourceText):
          appState.showFinalOptimizationFailure(message, sourceText: sourceText)
          self.hotkeyManager.isProcessing = true
        ...
        ModeBinding(
          ...
          onStart: { [weak self] in
            ...
            if phase == .recording || phase == .preparing {
              NSLog(
                "[Type4Me] >>> HOTKEY: toggle desync – onStart while recording, redirecting to STOP (phase=%@)",
                String(describing: phase))
              DebugFileLogger.log(
                "hotkey toggle desync: onStart while recording, redirecting to stop phase=\(phase)")
  ...
  private func toggleSelectionAskFollowUp(conversationContext: String) -> Bool {
    ...
    let availableModes = appState.availableModes
    let resolvedMode = ASRProviderRegistry.resolvedMode(
      for: .selectionAsk, provider: selectedProvider)
    let effectiveMode = availableModes.first(where: { $0.id == resolvedMode.id }) ?? resolvedMode
  ...
  private func handleHotkeyRetry(_ timer: Timer) {
    if PermissionManager.hasAccessibilityPermission {
      ...
      if ok {
        ...
        hotkeyRetryCount = 0
        NSLog(
          "[Type4Me] Accessibility granted but hotkey tap failed after retries. Suggesting restart."
        )
        showRestartAlert()
      }
    }
  ...
  private func showRestartAlert() {
    let alert = NSAlert()
    alert.messageText = L(
      "辅助功能权限已开启，但快捷键未生效", "Accessibility is enabled, but the hotkey is not working")
