...
enum TF {
  ...
  /// Dark tint over the frosted material. Light enough that the wallpaper's
  /// hue still bleeds through — these should read as glass, not as painted
  /// panels — but no lighter: at 0.42 a white document pushed the composited
  /// surface to ~0.46 luminance and body text fell to 3.1:1, metadata to
  /// 1.8:1. 0.55 puts body at 4.4:1 over pure white and costs nothing on the
  /// dark backdrops where everything already passed.
  static let frostTint = Color.black.opacity(0.55)
  ...
  /// Metadata, column headers, placeholder copy. Not lower than this: 0.34
  /// measured 1.8:1 over a bright backdrop, and this token carries the column
  /// titles, the device chip and every metadata string — structural text that
  /// has to survive a light wallpaper.
  static let frostTextFaint = Color.white.opacity(0.45)
