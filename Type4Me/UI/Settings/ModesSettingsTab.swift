...
struct ModesSettingsTab: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      SettingsSectionHeader(
        ...
        title: L("处理模式", "Modes"),
        description: L(
          "配置语音转写与后处理流水线。快速模式实时输出，自定义模式可经 LLM 加工。",
              ...
              HStack(spacing: 4) {
                Button {
                  ...
                  HStack(spacing: 4) {
                    ...
                    Text(
                      HotkeyRecorderView.keyDisplayName(
  ...
  private var selectionAskExamples: [String] {
    [
      ...
      L(
        "按下「随便问」热键，说出“翻译一下”“这是什么意思”“帮我总结”等问题。",
        "Press the Ask Anything hotkey and say a question such as translate this, what does this mean, or summarize it."
      ),
      L(
        "再次按热键停止，在弹框中阅读流式 Markdown 回答。",
        "Press the hotkey again to stop and read the streamed Markdown answer in the popup."),
    ]
  }
...
private struct HotkeyRecordingSheet: View {
    ...
    VStack(spacing: 20) {
      ...
      if let conflict {
        HStack(spacing: 4) {
          ...
          Text(
            L(
              "「\(conflict.name)」正在使用此快捷键，确认后将移除其绑定",
  ...
  private func startListening() {
    ...
    eventMonitor = NSEvent.addLocalMonitorForEvents(matching: [
      .flagsChanged, .keyDown, .otherMouseDown, .systemDefined,
    ]) { event in
      ...
      if event.type == .keyDown {
        ...
        if kc == 53
          && event.modifierFlags.intersection(.deviceIndependentFlagsMask).subtracting([
  ...
  private func sanitizedModifierFlags(
    _ flags: NSEvent.ModifierFlags, forKeyCode keyCode: Int? = nil
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 4) {
        ...
        Text(
          L(
            "处理进行时浮窗显示的文案，如「翻译中」「修正中」",
            "Text shown in the floating bar during processing, e.g. \"Translating\" \"Correcting\"")
        )
        .font(.system(size: 10))
        .foregroundStyle(TF.settingsTextTertiary)
      }
